...
import { roadTexture, checkerTexture, kerbTexture, textTexture } from './textures.js';
...
export function buildTrackMeshes(track, style, maxAniso) {
  const group = new THREE.Group();
  const hw = track.halfWidth;

  // Road
  const rt = roadTexture(style.road);
  rt.anisotropy = maxAniso;
  const roadMat = new THREE.MeshStandardMaterial({
    map: rt,
    roughness: style.road.roughness ?? 0.85,
    metalness: style.road.metalness ?? 0.0,
    roughnessMap: rt.userData.roughness || null,
  ...
  const shoulderMat = new THREE.MeshStandardMaterial({ color: style.shoulder ?? 0x3a3835, roughness: 0.95 });
  ...
  if (style.embankment) {
    const eMat = new THREE.MeshStandardMaterial({ color: style.embankment, roughness: 1 });
    for (const side of [-1, 1]) {
  ...
  if (bs.type === 'wall') {
    const wMat = new THREE.MeshStandardMaterial({ color: bs.color, roughness: 0.8, metalness: 0.05 });
    for (const side of [-1, 1]) {
